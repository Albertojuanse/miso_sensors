...
@discussion This method returns the ranger in ThetaThetaLocating mode.
*/
...
- (NSInteger)whileEditingTableItems:(UITableView *)tableView
              ...
              numberOfRowsInSection:(NSInteger)section
{
    // In ThetaThetaLocating, any device or new positions can be positioned.
...
- (UITableViewCell *)whileEditingTableItems:(UITableView *)tableView
                          ...
                          forRowAtIndexPath:(NSIndexPath *)indexPath
{
    // In ThetaThetaLocating, any device or new positions can be positioned, so an aditional row was added.
    // If one of these items have already got a position assigned, that position must be transferred to another item
                ...
                if ([selectedCell accessoryType] == UITableViewCellAccessoryDetailButton) { // If detailed
                    ...
                    [viewController dismissViewControllerAnimated:NO completion:^(void)
                    {
                        ...
                        [[NSNotificationCenter defaultCenter]
                         ...
                         object:self
                         userInfo:dataDic];
                        NSLog(@"[NOTI]%@ Notification \"vcEditing/presentMeasureView\" posted.", ERROR_DESCRIPTION_VCETTL);
                    }
                     ];
...
-(BOOL)processItemChosenByUser
{
    ...
    if (itemChosenByUser) {
        ...
        if (!position) {
            NSLog(@"[ERROR]%@ No position was found in the item chosen by user as device position; (0,0,0) is set.", ERROR_DESCRIPTION_VCETTL);
        ...
        if (!uuid) {
            NSLog(@"[ERROR]%@ No UUID was found in the item chosen by user as device position; a random one set.", ERROR_DESCRIPTION_VCETTL);
                       ...
                       ofTableItems:(UITableView *)tableView
{
    ...
    if (savedItem) {
        ...
        if (savedItemDic) {
            ...
            NSLog(@"[ERROR]%@ New position %@ could not be stored as an item.", infoDic[@"position"], ERROR_DESCRIPTION_VCETTL);
        }
        [sharedData inSessionDataSetAsNotChosenItem:itemDic
                                  toUserWithUserDic:userDic
                             withCredentialsUserDic:credentialsUserDic];
...
@discussion This method returns the image for measuring interface in thetaThetaLocating mode to measure the selected item.
*/
...
@discussion This method returns the behaviour when user taps 'Measure' button in Add view in ThetaThetaLocating mode to measure the selected item.
*/
...
@discussion This method returns the behaviour when the notification "rangingMeasureFinished" is recived in Add view in ThetaThetaLocating mode to measure the selected item.
*/
...
@discussion This method returns the behaviour when the notification "rangingMeasureFinishedWithErrors" is recived in Add view in ThetaThetaLocating mode to measure the selected item.
*/
                                                  ...
                                                  withMeasureButton:(UIButton *)measureButton
{
    ...
    if (consecutiveInvalidMeasuresError) {
        if ([consecutiveInvalidMeasuresError isEqualToString:@"consecutiveInvalidMeasures"]) {
            ...
            // Notify
            NSLog(@"[NOTI]%@ Notification \"lmd/rangingMeasureFinishedWithErrors\" posted.", ERROR_DESCRIPTION_VCETTL);
            [[NSNotificationCenter defaultCenter]
