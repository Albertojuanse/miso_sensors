            ...
            for (id positionKey in positionKeys) {
                ...
                if (savedItem) {
                    ...
                    NSMutableArray * itemDics = [sharedData fromItemDataGetItemsWithIdentifier:infoDic[@"identifier"]
                                                                         andCredentialsUserDic:credentialsUserDic];
                    if (itemDics.count == 0) {
                        NSLog(@"[ERROR][MM] Saved item %@ could not be retieved from shared data.", infoDic[@"identifier"]);
                    } else {
                        if (itemDics.count > 1) {
                            NSLog(@"[ERROR][MM] More than one saved item with identifier %@.", infoDic[@"identifier"]);
                        }
                    }
