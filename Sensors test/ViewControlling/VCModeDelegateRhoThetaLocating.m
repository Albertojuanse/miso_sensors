...
@discussion This method returns the ranger in RhoThetaLocating mode.
*/
              ...
              numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        NSInteger beaconsCount = [[sharedData fromItemDataGetItemsWithSort:@"beacon" andCredentialsUserDic:credentialsUserDic] count];
        NSInteger emptyPositionsCount = [[sharedData fromItemDataGetItemsWithSort:@"empty_position" andCredentialsUserDic:credentialsUserDic] count];
                ...
                if ([selectedCell accessoryType] == UITableViewCellAccessoryDetailButton) { // If detailed
                    ...
                    [viewController dismissViewControllerAnimated:NO completion:^(void)
                    {
                        ...
                        [[NSNotificationCenter defaultCenter]
                         ...
                         object:self
                         userInfo:dataDic];
                        NSLog(@"[NOTI]%@ Notification \"vcEditing/presentMeasureView\" posted.", ERROR_DESCRIPTION_VCERTL);
                    }
                     ];
                }
            }
                       ...
                       ofTableItems:(UITableView *)tableView
{
    ...
    if (savedItem) {
        ...
        if (savedItemDic) {
            ...
            NSLog(@"[ERROR]%@ New position %@ could not be stored as an item.", infoDic[@"position"], ERROR_DESCRIPTION_VCERTL);
        }
        [sharedData inSessionDataSetAsNotChosenItem:itemDic
                                  toUserWithUserDic:userDic
                             withCredentialsUserDic:credentialsUserDic];
    ...
    NSLog(@"[NOTI]%@ Notification \"canvas/refresh\" posted.", ERROR_DESCRIPTION_VCERTL);
    [[NSNotificationCenter defaultCenter]
...
@discussion This method returns the image for measuring interface in RhoThetaLocating mode to measure the selected item.
*/
...
@discussion This method returns the behaviour when user taps 'Measure' button in Add view in RhoThetaLocating mode to measure the selected item.
*/
...
@discussion This method returns the behaviour when the notification "rangingMeasureFinished" is recived in Add view in RhoThetaLocating mode to measure the selected item.
*/
...
@discussion This method returns the behaviour when the notification "rangingMeasureFinishedWithErrors" is recived in Add view in RhoThetaLocating mode to measure the selected item.
*/
                                                  ...
                                                  withMeasureButton:(UIButton *)measureButton
{
    ...
    if (consecutiveInvalidMeasuresError) {
        if ([consecutiveInvalidMeasuresError isEqualToString:@"consecutiveInvalidMeasures"]) {
            ...
            // Notify
            NSLog(@"[NOTI]%@ Notification \"lmd/rangingMeasureFinishedWithErrors\" posted.", ERROR_DESCRIPTION_VCERTL);
            [[NSNotificationCenter defaultCenter]
